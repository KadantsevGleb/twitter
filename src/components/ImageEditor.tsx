...
import { SettingsType } from "./Share"
...
const ImageEditor = ({ onClose, previewURL, setSettings, settings }: ImageEditorProps) => {
	...
	function handleChangeType(type: SettingsType['type']) {
		setSettings(prev => ({
			...prev,
			type
		}))
	...
	return (
		<div className="fixed w-screen h-screen left-0 top-0 bg-black/70 z-10 flex items-center justify-center">
			<div className="bg-black rounded-xl p-12 flex flex-col gap-4">
				{/* TOP */}
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-8">
						<X onClick={onClose} className="cursor-pointer" size={26}/>
						<h1 className="font-bold text-xl">Media Settings</h1>
					</div>
					<button
						className="py-2 px-4 bg-white text-black font-bold rounded-full cursor-pointer"
						onClick={onClose}
					>
						Save
					</button>
				...
				{/* IMAGE CONTAINER */}
				<div className="h-[600px] w-[600px] flex items-center relative">
					<Image
						src={previewURL}
						alt=""
						width={600}
